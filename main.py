...
def Init():
    ...
    modelResponse=requests.get(url=modelJson["assets"][2]["browser_download_url"],headers=secondHeaders)
    global clf
    clf=pickle.load(io.BytesIO(modelResponse.content))
    ...
    secondHeaders={'Accept': 'application/octet-stream' }
    meanStdResponse=requests.get(url=meanStdJson["assets"][1]["browser_download_url"],headers=secondHeaders)
    global df
    df= pd.read_csv(io.BytesIO(meanStdResponse.content),sep=",",index_col=0)
    ...
    if response.status_code==404:
        ...
        for k in mapDifficulty:
            ...
            if k["characteristic"]!="Standard":
                ...
                i=0
                list=[]
                for d in [bpm,duration,difficulty,sageScore,njs,offset,notes,bombs,obstacles,
                    nps,events,chroma,warns,resets]:
                    d=(d-df.iloc[i,0])/df.iloc[i,1]
                    list.append(d)
                    i+=1
                ...
                finishPredict=(ans[0]*df.iloc[14,1])+df.iloc[14,0]
